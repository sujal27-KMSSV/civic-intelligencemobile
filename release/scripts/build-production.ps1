...
try {
    ...
    Copy-Item build\app\outputs\flutter-apk\app-release.apk release\FixMyGrid.apk -Force
    ...
    Copy-Item build\app\outputs\bundle\release\app-release.aab release\FixMyGrid.aab -Force

    Write-Host "OK: release\FixMyGrid.apk and release\FixMyGrid.aab written."
}
finally {
    Pop-Location
}